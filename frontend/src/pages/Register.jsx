      ...
      <div className="relative w-full max-w-[440px] animate-fade-up">
        ...
        <div className="surface-raised rounded-2xl p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-5">
            ...
            <div className="flex flex-wrap gap-2 p-3 rounded-xl bg-canvas-100 border border-canvas-200">
              {['6+ characters', 'One number', 'Passwords match'].map((req) => (
                <span key={req} className="badge badge-indigo">
            ...
            <button type="submit" disabled={isLoading} className="btn-primary w-full">
              {isLoading ? <><Loader size="sm" /> Creating account…</> : 'Create account'}
            </button>
          </form>
          ...
          <p className="text-center text-sm text-ink-500 font-body">
            ...
            <Link to="/login" className="text-indigo-600 hover:text-indigo-800 font-display font-semibold transition-colors">
              Sign in
            </Link>
          </p>
        </div>
